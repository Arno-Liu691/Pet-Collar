#!/usr/bin/env python3
"""Comprehensive V15/V17 regression evaluation for the supplied recordings."""
...
for dataset in DATASETS:
    frames[dataset] = {}
    for algorithm, filename in [("V15", "v15_results.csv"), ("V17", "v17_results.csv")]:
...
metrics.to_csv(ROOT / "metrics_summary_v17.csv", index=False)
pd.DataFrame(lag_rows).to_csv(ROOT / "hr_lag_diagnostic_v17.csv", index=False)

# SQI calibration: high SQI should progressively reduce large errors.
sqi_rows = []
for dataset in DATASETS:
    for algorithm in ["V15", "V17"]:
...
pd.DataFrame(sqi_rows).to_csv(ROOT / "hr_sqi_calibration_v17.csv", index=False)

# Verify that HR-only V17 work did not alter RR or the rest/motion state machine.
invariant_columns = [
...
invariant_rows = []
for dataset in DATASETS:
    first = frames[dataset]["V15"]
    second = frames[dataset]["V17"]
...
pd.DataFrame(invariant_rows).to_csv(ROOT / "rr_rest_invariance_v17.csv", index=False)
...
pd.DataFrame(motion_rows).to_csv(ROOT / "motion_segment_summary_v17.csv", index=False)

# V17 feature-use summary.
feature_rows = []
for dataset in DATASETS:
    frame = frames[dataset]["V17"]
...
pd.DataFrame(feature_rows).to_csv(ROOT / "hr_evidence_usage_v17.csv", index=False)
...
for dataset in DATASETS:
    ...
    axes[0].plot(v17["algorithm_time_s"], v17["hr_reference"], label="Reference HR", linewidth=2.0)
    axes[0].plot(v17["algorithm_time_s"], v17["hr_reference_trailing18"],
    ...
    axes[0].plot(v17["algorithm_time_s"], v17["hr_bpm"].where(v17["hr_reportable"].eq(1)),
                 label="V17 reportable", linewidth=1.7)
    axes[0].set_ylabel("HR (bpm)")
    axes[0].set_title(f"{dataset}: V15 versus V17 HR")
    axes[0].grid(alpha=0.25)
    axes[0].legend(ncol=2)

    absolute_error = (v17["hr_bpm"] - v17["hr_reference"]).abs()
    axes[1].plot(v17["algorithm_time_s"], absolute_error, label="V17 absolute error", color="tab:red")
    axes[1].axhline(8, color="tab:red", linestyle="--", linewidth=1, alpha=0.6)
    sqi_axis = axes[1].twinx()
    sqi_axis.plot(v17["algorithm_time_s"], v17["hr_sqi"], label="V17 HR SQI", color="tab:green", alpha=0.8)
    sqi_axis.axhline(65, color="tab:green", linestyle=":", linewidth=1)
    axes[1].set_ylabel("Absolute error (bpm)")
    sqi_axis.set_ylabel("SQI")
    axes[1].grid(alpha=0.2)

    axes[2].plot(v17["algorithm_time_s"], v17["hr_cand1_bpm"], label="Candidate 1", alpha=0.65)
    axes[2].plot(v17["algorithm_time_s"], v17["hr_cand2_bpm"], label="Candidate 2", alpha=0.45)
    axes[2].plot(v17["algorithm_time_s"], v17["hr_cand3_bpm"], label="Candidate 3", alpha=0.35)
    consensus = v17["hr_consensus_used"].eq(1)
    axes[2].scatter(v17.loc[consensus, "algorithm_time_s"], v17.loc[consensus, "hr_candidate_bpm"],
    ...
    figure.tight_layout()
    figure.savefig(PLOTS / f"{dataset}_hr_v17.png", dpi=180)
    plt.close(figure)
